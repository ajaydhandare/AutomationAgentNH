...
/// credentials and the service-token endpoint in one call — and because the token is cached, a
/// frequent health probe costs nothing after the first.
/// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return string.IsNullOrWhiteSpace(token)
                ? HealthCheckResult.Unhealthy("ERP returned an empty service token.")
                : HealthCheckResult.Healthy("ERP API reachable and the service token is valid.");
        }
        catch (Exception ex)
        {
