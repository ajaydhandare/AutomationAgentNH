...
public sealed class ServiceTokenTests
{
    ...
    {
        var token = new ServiceToken("t", Now.AddMinutes(60));
    ...
    public void A_token_inside_the_refresh_margin_is_already_treated_as_stale()
    {
        ...
        var token = new ServiceToken("t", Now.AddSeconds(90));
    ...
    {
        var token = new ServiceToken("t", Now + ServiceToken.RefreshMargin + TimeSpan.FromSeconds(1));
        token.IsUsableAt(Now).Should().BeTrue();

        var atBoundary = new ServiceToken("t", Now + ServiceToken.RefreshMargin);
    ...
    {
        var token = new ServiceToken("t", Now.AddMinutes(-1));
